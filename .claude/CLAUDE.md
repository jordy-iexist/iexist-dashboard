...
- **Database pattern**: Direct Supabase Python client usage (no ORM)
- **Task queuing**: Each CSV row becomes a separate Celery job
- **Status flow**: `pending` → `processing` → `completed`/`failed`

**Key backend modules**:
- `app/main.py` - FastAPI app with CORS and endpoints
- `app/database.py` - Supabase client initialization
- `app/services/csv_service.py` - CSV parsing and storage logic
...
### Database Operations
- Backend uses Supabase Python client directly (no ORM like SQLAlchemy)
- All database logic is in `app/database.py` and service files
- MCP tools can query Supabase directly when server is running
