...
def generate_blog_task(self, job_id: str):
    ...
    try:
        ...
        if not csv_row:
            raise ValueError(f"CSV rij voor job {job_id} niet gevonden.")

        row_data = csv_row.data
        if not isinstance(row_data, dict):
            raise ValueError(f"CSV data voor job {job_id} is ongeldig.")
        ...
        if not created_by:
            raise ValueError(f"Gebruiker ontbreekt voor blog generatie job {job_id}.")
