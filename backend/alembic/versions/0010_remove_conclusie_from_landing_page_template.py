"""remove conclusie line from landing page upload template
...
def upgrade() -> None:
    lpu = sa.table("landing_page_uploads", sa.column("template", sa.Text()))
    op.execute(
        lpu.update().where(lpu.c.template == OLD_TEMPLATE).values(template=NEW_TEMPLATE)
    )
...
def downgrade() -> None:
    lpu = sa.table("landing_page_uploads", sa.column("template", sa.Text()))
    op.execute(
        lpu.update().where(lpu.c.template == NEW_TEMPLATE).values(template=OLD_TEMPLATE)
    )
